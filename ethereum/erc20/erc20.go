...
// ContractsWithdrawalIterator is returned from FilterWithdrawal and is used to iterate over the raw logs and unpacked data for Withdrawal events raised by the Contracts contract.
type ContractsWithdrawalIterator struct {
	Event *ContractsWithdrawal // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ContractsWithdrawalIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ContractsWithdrawal)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ContractsWithdrawalIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ContractsWithdrawalIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ContractsWithdrawal represents a Withdrawal event raised by the Contracts contract.
type ContractsWithdrawal struct {
	Src common.Address
	Wad *big.Int
	Raw types.Log // Blockchain specific contextual infos
}
...
func (_Contracts *ContractsFilterer) FilterWithdrawal(opts *bind.FilterOpts, src []common.Address) (*ContractsWithdrawalIterator, error) {
	...
	logs, sub, err := _Contracts.contract.FilterLogs(opts, "Withdrawal", srcRule)
	if err != nil {
		return nil, err
	}
	return &ContractsWithdrawalIterator{contract: _Contracts.contract, event: "Withdrawal", logs: logs, sub: sub}, nil
}

// WatchWithdrawal is a free log subscription operation binding the contract event 0x7fcf532c15f0a6db0bd6d0e038bea71d30d808c7d98cb3bf7268a95bf5081b65.
//
// Solidity: event Withdrawal(address indexed src, uint256 wad)
func (_Contracts *ContractsFilterer) WatchWithdrawal(opts *bind.WatchOpts, sink chan<- *ContractsWithdrawal, src []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _Contracts.contract.WatchLogs(opts, "Withdrawal", srcRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ContractsWithdrawal)
				if err := _Contracts.contract.UnpackLog(event, "Withdrawal", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Withdrawal(address indexed src, uint256 wad)
func (_Contracts *ContractsFilterer) ParseWithdrawal(log types.Log) (*ContractsWithdrawal, error) {
	event := new(ContractsWithdrawal)
	if err := _Contracts.contract.UnpackLog(event, "Withdrawal", log); err != nil {
		return nil, err
	}
